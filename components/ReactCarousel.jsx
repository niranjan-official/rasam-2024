    ...
    return (
        <Carousel responsive={responsive} className='w-full flex h-full' partialVisible={true} infinite={true} autoPlay={true} autoPlaySpeed={3000} removeArrowOnDeviceType={['mobile','tablet']}>
            {CarouselImages.map((obj, index) => (
                ...
                <button onClick={()=>router.push(`/events/${index}`)} className='absolute left-1/2 bottom-5 xl:left-1/3 transform -translate-x-1/2 -translate-y-1/2 p-2 px-4 backdrop-filter backdrop-blur-lg text-lg shadow-md rounded-2xl'>Know More</button>
              </div>
            ))}
        </Carousel>
    )
